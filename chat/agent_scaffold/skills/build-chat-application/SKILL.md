...
1. Read `node_modules/@dev-mainsequence/chat/package.json`: the version, the `exports`
...
2. Install the chat next to the SDK it already uses:
   ...
   ```bash
   npm install @dev-mainsequence/chat @dev-mainsequence/command-center-sdk react react-dom
   ```
...
3. Import only `@dev-mainsequence/chat` and `@dev-mainsequence/chat/styles.css`. Never import
...
The package ships its standalone application in `node_modules/@dev-mainsequence/chat/standalone/`:
