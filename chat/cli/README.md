...
This directory is the Node-only command line of `@dev-mainsequence/chat`: it installs the
...
- `mainsequence-chat.mjs`, the package's `mainsequence-chat` binary:
  `mainsequence-chat skills install [--path <repository-root>]`.
...
- The destination is `<repository-root>/.agents/skills/chat/`. The package owns that namespace
...
validates every packaged skill. Run them with `npm --workspace @dev-mainsequence/chat run test`.
