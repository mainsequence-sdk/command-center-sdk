...
A pure reducer in `packages/chat/src/engine/message-queue.ts` implements `enqueue`,
...
### `packages/chat/src/engine/message-queue.ts` (new)
...
### `packages/chat/src/engine/ChatEngineProvider.tsx`
