...
const cliPath = join(packageRoot, "cli", "mainsequence-chat.mjs");
...
test("installs every skill into its own namespace and leaves every other namespace alone", async (t) => {
  ...
  assert.equal(result.destinationRoot, join(await realpathOf(project), ".agents", "skills", "chat"));
  ...
  assert.match(provenance, /^namespace=chat$/mu);
...
test("replaces the whole namespace on every install", async (t) => {
  const project = await temporaryRepository(t);
  const namespace = join(project, ".agents", "skills", "chat");
...
test("refuses a destination that is a symbolic link and writes nothing through it", async (t) => {
  ...
  await symlink(elsewhere, join(project, ".agents", "skills", "chat"));
  ...
  assert.deepEqual((await readdir(join(project, ".agents", "skills"))).sort(), ["chat"]);
});
