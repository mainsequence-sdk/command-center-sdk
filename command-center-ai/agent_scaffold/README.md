...
This directory is the versioned agent skill bundle shipped with `@dev-mainsequence/command-center-ai`. Its
...
The package's postinstall and `command-center-ai skills install --path .` copy every skill into
`<repository-root>/.agents/skills/command-center-ai/`, a namespace the package owns alone: every install
...
- A skill names only published entrypoints (`@dev-mainsequence/command-center-ai`,
  `@dev-mainsequence/command-center-ai/styles.css`, and the SDK's), never `src/`, `dist/`, or a repository path.
