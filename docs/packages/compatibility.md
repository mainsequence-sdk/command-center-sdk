...
| Chat's SDK peer range | A caret range on the first SDK release that publishes `--warning-tint` | A new SDK minor widens the range and releases the chat in the same change. |
