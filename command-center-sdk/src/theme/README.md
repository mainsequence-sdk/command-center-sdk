...
- `styles.css`: browser-ready base stylesheet, theme chrome variables, body typography, and reusable
  ...
  a coarse pointer), `--application-safe-area-top/right/bottom/left`,
  `--application-navigation-rail-width`, and the host-settable
  `--application-navigation-drawer-width` (20rem), and drops `background-attachment: fixed` on
