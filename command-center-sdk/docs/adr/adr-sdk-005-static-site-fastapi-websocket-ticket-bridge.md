...
The platform removes both platform values before the request reaches the FastAPI application. The
browser receives the application subprotocol selected by the FastAPI route, or the fixed
acknowledgement when the route selects no application subprotocol, so it always receives exactly
one protocol it offered.
...
that calls the platform's ticket endpoint and injects the resolver into each managed static-site
viewer. The platform remains the authorization authority, and once the connection is upgraded
neither the platform API nor Command Center is in the WebSocket data path.

## Current-State Verification

This decision was checked against the package registry on 2026-09-13:

- npm's `latest` dist-tag for `@dev-mainsequence/command-center-sdk` is `0.1.21`;
- the SDK repository's current `main` package metadata is the not-yet-published `0.2.0` line; and
...
ResourceRelease, image, hostname, and process. There is no separate WebSocket release kind or
deployment declaration. The browser-side authentication mechanisms are nevertheless different:
...
WebSocket handshake
  ...
  platform -> FastAPI/Starlette route with only appProtocols
  browser <- selected appProtocol, or bridge-ack when none was selected
  browser <==================== upgraded connection ====================> FastAPI release
```
...
Command Center, as the host, owns:

1. using its normal authenticated API client to call the platform's public ticket endpoint;
...
The platform owns ticket issuance, authorization, Origin and path policy, expiry, single-use
consumption, removal of both platform subprotocols before the request reaches the FastAPI
application, and the per-request user identity the application receives. The FastAPI application
owns its routes, messages, close behavior, and optional application subprotocols.

## Accepted Backend Contract

The Command Center resolver calls the normal authenticated platform endpoint:
...
A ticket is short-lived; `expires_at` is authoritative. The ticket is for one handshake, not one
connection lifetime. The first handshake that presents it consumes it, even if the connection then
fails. An already upgraded connection may outlive the ticket expiry.

The complete reserved authentication subprotocol must match this canonical grammar:
...
The acknowledgement is not a credential and is not returned by the ticket endpoint. It is a protocol
marker defined by this contract. The browser WebSocket algorithm fails a handshake when the
constructor was given one or more protocols but the `101` response does not select exactly one
offered protocol. The platform therefore answers every browser-ticket handshake with exactly one
`Sec-WebSocket-Protocol` value: the application protocol the FastAPI route selected, or
`mainsequence.ws-bridge.v1` when the route selected none. It never selects or echoes the reserved
...
- `/_healthz` and `/logos/...` are rejected as reserved platform paths;
- the child does not send Origin, source identity, user identity, organization, environment,
  release URL, ticket, or application subprotocols; and
...
`invalid_request`. In particular, the ticket endpoint's `websocket_ticket_store_unavailable`,
unclassified `5xx`, and transport failures map to `temporarily_unavailable`; raw backend bodies and
internal metadata are never forwarded.
...
second. The platform removes both before the request reaches the FastAPI application, so the
application sees and negotiates only the caller's application protocols.

Every successful browser-ticket `101` selects exactly one protocol. When the FastAPI route selects
an application protocol, the platform returns that value. When the route selects none, including
when the caller supplied no application protocols, the platform returns `mainsequence.ws-bridge.v1`.
Consequently `socket.protocol` is either the selected application protocol or the fixed
acknowledgement; it is never the ticket. A response with no selected protocol, the ticket protocol,
more than one protocol, or an unoffered protocol fails the browser handshake.
...
This rule follows from single use: the first handshake attempt may spend the ticket before the
connection succeeds. Automatic retry with the same ticket would be both
...
The platform removes it before the request reaches the FastAPI application; project code receives
only the platform-injected user identity, never the ticket.
...
`unsupported`/timeout behavior. The SDK does not call the platform directly because the embedded
site does not own the Command Center session and must not infer an auth endpoint.
...
| New | New with resolver | WebSocket bridge is available |
...
8. map platform and network failures to the SDK's narrow error codes without returning raw bodies;
...
| Ticket endpoint result | SDK code |
| --- | --- |
| Explicit `400 invalid_request` or another `400` validation error | `invalid_request` |
...
when no resolver is installed; that code is not expected from the platform.
...
5. exact backend-result binding checks for UID, Origin, path, URL, expiry, and the complete reserved
   subprotocol grammar;
6. ticket-first and acknowledgement-second ordering; removal of both before the FastAPI application;
   preservation of zero, one, and multiple valid application protocols; application selection by
   the route; and acknowledgement selection when the route selects none;
...
12. real cross-origin browser tests proving that zero application protocols opens with
    `socket.protocol === mainsequence.ws-bridge.v1`, an application protocol selected by the route
    is preserved, and omission or duplication of the selected protocol fails the handshake, in
    addition to `Origin`, messages, and close behavior; and
13. `npm run check`, `npm pack --dry-run`, packed artifact inspection, and an external TypeScript
...
1. Implement this ADR in the SDK, complete its full maintenance cycle, and publish the first
   package version that actually contains the new APIs, protocol schema, documentation, skills,
   and tests.
2. Pin Command Center to that exact version and inject the authenticated resolver into every
   managed viewer once the platform issues WebSocket tickets for every FastAPI target those viewers
   may expose.
3. Canary an embedded site that serves REST and WebSocket routes from one FastAPI release, then
   broaden rollout.
...
### Let the embedded child call the platform directly
...
reporting. The platform accepts only the reserved subprotocol transport for browsers.
...
`Sec-WebSocket-Protocol`, even though the ticket was consumed and the FastAPI application accepted
the connection.
...
acknowledgement satisfies the browser handshake without exposing the credential or preventing a
route-selected application protocol from being returned.

### Proxy WebSocket frames through Command Center or the platform API

Rejected because neither belongs in the WebSocket data path. After the `101` upgrade, the browser is
connected directly to the FastAPI release.

### Cache or automatically retry tickets

Rejected because tickets are single-use and may be consumed before the connection is
...
Rejected because every FastAPI release supports WebSockets; REST and WebSocket routes deploy
together in one existing FastAPI release.
...
general credential or making Command Center or the platform API a frame proxy. The additional cost
is one uncached authenticated ticket issue and validation per connection attempt. Applications must
own reconnect and state resynchronization, while the SDK owns the safe creation boundary.
...
evaluated when the ticket is consumed at the handshake; an already upgraded connection is not
continuously revalidated. Command Center must dispose the viewer on local authentication-session
...
is not inferred by the SDK or the platform.
...
- The reserved ticket subprotocol is first and the fixed non-secret acknowledgement is second; both
  are removed before the request reaches the FastAPI application, and the ticket is never selected
  or exposed as a raw public value.
- Every successful browser-ticket `101` returns exactly one protocol offered by the browser: the
  application protocol selected by the FastAPI route, or `mainsequence.ws-bridge.v1` when the route
  selected none. Zero-application-protocol connections open successfully.
...
- REST and WebSocket routes work from one FastAPI release without a new release kind, deployment
  field, pipeline, image, service, or hostname.
...
- [ ] Command Center's authenticated resolver integrated into every managed viewer.
- [ ] Exact package version containing the contract published and consumed by Command Center.
- [ ] End-to-end browser canary passing against the platform.
